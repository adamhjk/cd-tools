...
function merge_change {
    ...
    until git fetch ssh://jenkins@$SITE:29418/$PROJECT.git $REFSPEC
    do
        COUNT=$(($COUNT + 1))
        logger -p user.warning -t 'gerrit-git-prep' FAILED: git fetch git fetch ssh://jenkins@$SITE:29418/$PROJECT.git $REFSPEC COUNT: $COUNT
